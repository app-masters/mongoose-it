...
module.exports = function (schema) {
    ...
    schema.statics.findItByIdCache = async function (id, populate) {
        ...
        if (record && record.constructor.name==="Array"){
            console.log("...... received array when must be just a single object.. converting, but, must check it on mongooseit...");
            // console.log("findOne result", record.constructor.name);
    ...
    schema.methods.cacheIt = function () {
        if (this.constructor.name!=="model"){
            throw new Error("Calling cacheIt in a object, not a model");
        }
        let key = getModelKey(this.constructor.modelName, this._id);
